...
int main(int argc, char * argv[])
{
  if (argc != 6){
    printf("Usage: rosrun xsens_mti_driver sampling_frequency enable_acceleration(0-1) "
           "enable_angular_velocity(0-1) enable_orientation(0-1) enable_baro\n\n");
  ...
  interface.configureOutput(std::atoi(argv[1]), std::atoi(argv[2]), std::atoi(argv[3]), std::atoi(argv[4]), std::atoi(argv[5]));
